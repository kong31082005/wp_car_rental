...
?>
  ...
  <script>
    ...
    document.getElementById('rentBtn').addEventListener('click', function() {
      ...
      const orderData = {
        action: 'create_car_rental_order',
        car_id: <?php echo get_the_ID(); ?>,
        pickup_time: document.getElementById('pickupTime').value,
        return_time: document.getElementById('returnTime').value,
        pickup_type: selectedPickupType,
        delivery_address: deliveryAddress,
        public_coupon: selectedPublicCoupon ? selectedPublicCoupon.code : '',
        personal_coupon: selectedPersonalCoupon ? selectedPersonalCoupon.code : ''
      };

      // Gửi AJAX tới WordPress backend
      fetch('<?php echo admin_url('admin-ajax.php'); ?>', {
        ...
        .then(data => {
          if (data.success && data.data.checkoutUrl) {
            ...
            alert('Lỗi: ' + (data.data.message || 'Không thể tạo đơn hàng.'));
